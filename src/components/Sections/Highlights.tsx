...
const Highlights: FC = memo(() => {
  ...
  const {imageSrc, highlights} = HighlightSections;
  ...
  const next = useCallback(() => {
    if (activeIndex + 1 === highlights.length) {
      setTestimonial(0)();
    } else {
      setTestimonial(activeIndex + 1)();
    }
  }, [activeIndex, setTestimonial, highlights.length]);
...
const Hilight: FC<{highlight: Highlight; isActive: boolean}> = memo(({highlight: {text, name, image}, isActive}) => (
  <div
    className={classNames(
